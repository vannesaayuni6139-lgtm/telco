    ...
    <main className="max-w-7xl mx-auto px-4 py-8 md:py-12">
      ...
      <div className="bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-2xl border border-indigo-700/50 p-6 md:p-8 mb-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <Button className="bg-blue-600 hover:bg-blue-700 text-white rounded-full px-6 whitespace-nowrap">Subscribe Now</Button>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-800">
                <th className="text-left p-6 text-slate-400 uppercase tracking-wider text-sm">Package Details</th>
          ...
          {filteredPurchases.map((purchase) => (
            <div key={purchase.id} className="p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <Badge className={`${getStatusColor(purchase.status)} border capitalize text-xs`}>{purchase.status}</Badge>
              </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
        <p className="text-slate-400 text-sm">Page <span className="text-white">1</span> of <span className="text-white">{totalPages}</span></p>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="bg-slate-900 border-slate-700 text-white hover:bg-slate-800 disabled:opacity-50 rounded-lg">
            <ChevronLeft className="size-4 mr-1" />
            Previous
          </Button>
          <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="bg-slate-900 border-slate-700 text-white hover:bg-slate-800 disabled:opacity-50 rounded-lg">
            Next
